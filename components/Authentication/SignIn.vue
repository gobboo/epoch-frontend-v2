		...
		<div class="modal">
			<div class="modal-box px-12 w-96">
				...
				<div class="flex flex-col gap-4">
					<a v-for="(provider, index) in providers" :key="index" :class="`btn normal-case ${provider.disabled ? 'btn-disabled' : provider.color}`" :href="`${useRuntimeConfig().public.BASE_URL}/auth/${provider.name}`">
...
const providers = [
	{ name: 'Google', icon: 'fa6-brands:google', url: '', color: 'bg-white text-black', disabled: true },
	{ name: 'Discord', icon: 'fa6-brands:discord', url: '', color: 'bg-indigo-500 text-white', disabled: false },
	{ name: 'Steam', icon: 'fa6-brands:steam', url: '', color: 'bg-slate-900 text-white', disabled: false }
]
